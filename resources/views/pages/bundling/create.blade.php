...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Setting Product Bundling</h1>
    ...
    <form id="bundleForm" action="{{ route('bundlings.store') }}" method="POST" enctype="multipart/form-data" class="w-full space-y-6"
         x-data="{ 
             ...
             mainProduct: { product_id: '', variant_id: '', search: '' },
             suggestItems: [
                 { product_id: '', variant_id: '', bundle_price: '', discount_percent: '', search: '' }
             ],
             bundleName: '{{ old('name') }}',
             isNameCustom: {{ old('name') ? 'true' : 'false' }},
        ...
        <div class="bg-white rounded-xl shadow-xs border border-outline-variant/40 p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <!-- Varian Produk Utama (Opsional jika produk utama punya banyak varian) -->
                <div>
                    ...
                    <select name="items[0][variant_id]" x-model="mainProduct.variant_id" class="w-full px-3 py-2 text-xs border border-outline-variant rounded-lg focus:outline-none focus:border-primary bg-white">
                        ...
                        <template x-if="mainProduct.product_id">
                            <template x-for="v in (getProduct(mainProduct.product_id)?.variants || [])" :key="v.id">
                                <option :value="v.id" x-text="`${v.variant_name} (Rp ${Number(v.sell_price || v.price || 0).toLocaleString('id-ID')})`"></option>
                            </template>
                        </template>
                    </select>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-xs border border-outline-variant/40 p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" x-model="bundleName" @input="isNameCustom = true" class="w-full px-3 py-2 text-xs border border-outline-variant rounded-lg focus:outline-none focus:border-primary font-semibold" placeholder="Contoh: Bundling Kasur Matras + Sprei" required>
                </div>
                <div>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="btn-save px-5 py-2 bg-primary text-white rounded-lg text-xs font-bold hover:opacity-90">Simpan Bundling</button>
        </div>
    </form>
@endsection
