...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Create Brand</h1>
        ...
        <div class="w-full bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="brandName" name="name" value="{{ old('name') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g. Lady Americana" required>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="brandSlug" name="slug" value="{{ old('slug') }}" readonly class="w-full px-3 py-2 border border-outline-variant rounded-lg bg-surface-container-low/60 cursor-not-allowed text-on-surface-variant focus:outline-none" placeholder="Otomatis terisi dari nama brand">
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" rows="4" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Describe the brand...">{{ old('description') }}</textarea>
                ...
                <div class="flex flex-col gap-4">
                    <div class="space-y-1.5 md:w-1/3">
                        ...
                        <select name="banner_type" id="banner_type" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" onchange="toggleBannerInputs()">
                            <option value="1" {{ old('banner_type', 1) == 1 ? 'selected' : '' }}>Image Upload</option>
                            <option value="2" {{ old('banner_type', 1) == 2 ? 'selected' : '' }}>Image URL (Hotlink)</option>
                        </select>
                    </div>
                    ...
                    <div id="banner_url_inputs" class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 border border-outline-variant/30 rounded-xl bg-surface-container/30" style="display: none;">
                        <div class="space-y-1.5">
                            ...
                            <input type="url" name="embed_web" value="{{ old('embed_web') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="https://example.com/image-web.png">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="url" name="embed_mobile" value="{{ old('embed_mobile') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="https://example.com/image-mobile.png">
