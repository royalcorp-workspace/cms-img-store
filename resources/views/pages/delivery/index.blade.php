                ...
                <tbody class="divide-y divide-outline-variant/20">
                    @forelse($deliveries as $delivery)
                        <tr class="hover:bg-surface-container/30 transition-colors group">
                            <td class="px-6 py-4">
                                <span class="font-headline-md text-headline-md text-primary font-semibold">#{{ substr($delivery->id, 0, 8) }}</span>
                            </td>
                            <td class="px-6 py-4 font-body-md text-body-md text-on-surface">{{ substr($delivery->order->id, 0, 8) ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white text-label-sm font-bold">
                                        {{ strtoupper(substr($delivery->order->customer->name ?? 'N/A', 0, 2)) }}
                                    </div>
                            ...
                            <td class="px-6 py-4 text-center">
                                @php
                                    $statusMap = ['pending' => 'gray', 'in_transit' => 'indigo', 'delivered' => 'success', 'failed' => 'danger', 'returned' => 'orange'];
                                    $statusClass = 'bg-' . ($statusMap[$delivery->status] ?? 'gray') . '-100 text-' . ($statusMap[$delivery->status] ?? 'gray') . '-700';
                                @endphp
                            ...
                            <td class="px-6 py-4 text-on-surface-variant">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('delivery.show', $delivery->id) }}" class="p-1.5 hover:text-primary transition-colors" title="View"><span class="material-symbols-outlined text-[20px]">visibility</span></a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-on-surface-variant">No deliveries found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
