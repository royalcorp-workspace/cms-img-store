...
@section('title', 'Create Store Channel')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Create Store Channel</h1>
    ...
    <form method="POST" action="{{ route('store-channels.store') }}" class="space-y-6">
        @csrf
        <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="store_channel_group_id" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white" required>
                        ...
                        @foreach($channelGroups as $group)
                            <option value="{{ $group->id }}" {{ old('store_channel_group_id') == $group->id ? 'selected' : '' }}>{{ $group->name }} ({{ $group->code }})</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="code" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g., TOKO" required value="{{ old('code') }}">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g., Toko Offline" required value="{{ old('name') }}">
            ...
            <div class="space-y-1.5 mt-4">
                ...
                <textarea name="description" rows="2" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Optional description">{{ old('description') }}</textarea>
        ...
        <div class="flex justify-end gap-4">
            ...
            <button type="submit" class="px-10 py-3 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">Save Channel</button>
        </div>
    </form>
@endsection
