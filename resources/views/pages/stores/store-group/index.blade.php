...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Store Groups</h1>
        ...
        <a href="{{ route('store-groups.create') }}" class="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">
            <span class="material-symbols-outlined text-[18px]">add</span> Create Store Group
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    ...
                    <tr class="hover:bg-surface-container/30 transition-colors">
                        ...
                        <td class="px-gutter py-4">
                            <div class="flex gap-2">
                                <a href="{{ route('store-groups.edit', $group->id) }}" class="text-on-surface-variant hover:text-primary" title="Edit"><span class="material-symbols-outlined text-[18px]">edit</span></a>
                                <form action="{{ route('store-groups.destroy', $group->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this store group?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-gutter py-8 text-center text-on-surface-variant">No store groups found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-gutter py-4 border-t border-outline-variant bg-surface-container-low/30 flex justify-between items-center">
            <p class="font-body-md text-body-md text-on-surface-variant">Showing {{ $storeGroups->firstItem() ?? 0 }}-{{ $storeGroups->lastItem() ?? 0 }} of {{ number_format($storeGroups->total()) }} entries</p>
            <div class="flex gap-2">{{ $storeGroups->links() }}</div>
        </div>
    </div>
@endsection
