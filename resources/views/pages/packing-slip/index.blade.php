...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Packing Slip</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden mb-8">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-gray">
                        <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Packing ID</th>
                        ...
                        <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Box Count</th>
                    ...
                    @forelse($packingSlips as $slip)
                        <tr class="hover:bg-surface-container/30 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-center">
                                @php
                                    $statusMap = ['draft' => 'gray', 'packing' => 'indigo', 'packed' => 'success', 'cancelled' => 'danger'];
                                    $statusClass = 'bg-' . ($statusMap[$slip->status] ?? 'gray') . '-100 text-' . ($statusMap[$slip->status] ?? 'gray') . '-700';
                                @endphp
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-label-sm font-label-sm {{ $statusClass }}">
                                    <span class="w-1.5 h-1.5 rounded-full bg-current"></span> {{ ucfirst($slip->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 font-body-md text-body-md text-on-surface-variant">{{ $slip->box_count }}</td>
                            <td class="px-6 py-4 text-on-surface-variant">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('packing-slip.show', $slip->id) }}" class="p-1.5 hover:text-primary transition-colors" title="View"><span class="material-symbols-outlined text-[20px]">visibility</span></a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-on-surface-variant">No packing slips found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if($packingSlips->hasPages())
            <div class="px-6 py-4 border-t border-outline-variant bg-surface-container-low/30 flex justify-between items-center">
                <p class="font-body-md text-body-md text-on-surface-variant">Showing {{ $packingSlips->firstItem() }} to {{ $packingSlips->lastItem() }} of {{ $packingSlips->total() }} packing slips</p>
                <div class="flex gap-2">{{ $packingSlips->links() }}</div>
            </div>
        @endif
    </div>
@endsection