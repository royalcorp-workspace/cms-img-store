...
@section('title', 'Packing Slip Detail')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Packing Slip #{{ substr($packingSlip->id, 0, 8) }}</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div>
                ...
                <p class="font-body-md text-body-md text-on-surface">{{ ucfirst($packingSlip->status) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
        ...
        <div class="px-6 py-4 border-t border-outline-variant bg-surface-container-low/30">
            <a href="{{ route('packing-out.create', $packingSlip->id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg font-label-md text-label-md hover:opacity-90 transition-all">
                <span class="material-symbols-outlined text-[18px]">add</span> Create Packing Out
            </a>
        </div>
    </div>
@endsection