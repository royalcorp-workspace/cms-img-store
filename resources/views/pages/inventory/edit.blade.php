...
@section('title', 'Edit Stok Inventory')

@section('content')
    <!-- Header Section -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Edit Stok Inventory</h1>
            <nav class="flex items-center gap-2 text-label-sm text-on-surface-variant mt-1 font-medium">
                ...
                <span class="material-symbols-outlined text-[14px]">chevron_right</span>
                <span class="text-on-surface">Edit Stok</span>
            </nav>
        </div>
        <div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden max-w-4xl">
        <div class="p-6 border-b border-outline-variant/30">
            <h2 class="text-base font-semibold text-on-surface">Edit Data Alokasi Stok</h2>
            <p class="text-xs text-on-surface-variant mt-0.5">Perbarui nilai stok atau pindahkan lokasi alokasi gudang dan channel toko.</p>
        </div>

        <!-- Product & Variant Overview Card -->
        <div class="p-6 bg-surface-container-lowest border-b border-outline-variant/30">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 bg-surface-gray rounded-lg overflow-hidden flex-shrink-0 border border-outline-variant/30">
                    @php
                ...
                <div>
                    <h3 class="font-semibold text-base text-on-surface">{{ $product?->name ?? 'Produk' }}</h3>
                    <div class="mt-1 flex items-center gap-2 flex-wrap text-xs text-on-surface-variant">
                        @if($variant?->variant_name)
                            <span class="bg-primary/10 text-primary font-medium px-2 py-0.5 rounded border border-primary/20">
                                Varian: {{ $variant->variant_name }}
                            </span>
                        @endif
                        @if($variant?->sku)
                            <span class="bg-surface-container-low px-2 py-0.5 rounded border border-outline-variant/30 font-mono">
                                SKU: {{ $variant->sku }}
                            </span>
                        @endif
                    </div>
                    ...
                    <select name="store_channel_id" id="storeChannelSelect" required class="w-full select2-channel" data-placeholder="Pilih atau cari Store Channel..." data-ajax-url="{{ route('inventory.channels.search') }}">
                        ...
                        @foreach($channels as $ch)
                            <option value="{{ $ch->id }}" 
                                    ...
                                    data-code="{{ $ch->code }}"
                                    {{ old('store_channel_id', $inventory->store_channel_id) == $ch->id ? 'selected' : '' }}>
            ...
            <div class="border-t border-outline-variant/30 pt-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-3.5">
                    ...
                    <div class="p-3 bg-surface-container-low border border-outline-variant/40 rounded-xl">
                        ...
                        <input type="number" name="on_stock" id="inputOnStock" min="0" value="{{ old('on_stock', $inventory->on_stock ?? 0) }}" required class="w-full h-10 px-3 border border-outline-variant rounded-lg text-base font-bold text-on-surface bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none" oninput="calculateAvailable()">
                    ...
                    <div class="p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl">
                        ...
                        <input type="number" name="on_order" id="inputOnOrder" min="0" value="{{ old('on_order', $inventory->on_order ?? 0) }}" class="w-full h-10 px-3 border border-amber-300 rounded-lg text-base font-bold text-on-surface bg-white focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 focus:outline-none" oninput="calculateAvailable()">
                    ...
                    <div class="p-3 bg-purple-500/5 border border-purple-500/20 rounded-xl">
                        ...
                        <input type="number" name="outgoing" id="inputOutgoing" min="0" value="{{ old('outgoing', $inventory->outgoing ?? 0) }}" class="w-full h-10 px-3 border border-purple-300 rounded-lg text-base font-bold text-on-surface bg-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 focus:outline-none" oninput="calculateAvailable()">
                    ...
                    <div class="p-3 bg-success/10 border-2 border-success/30 rounded-xl flex flex-col justify-between">
                        <div>
                            ...
                            <div id="previewAvailable" class="text-2xl font-extrabold text-success mt-1">{{ $inventory->available ?? 0 }}</div>
                        </div>
