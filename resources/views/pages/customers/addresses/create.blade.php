...
@section('title', 'Tambah Alamat - ' . $customer->name)

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Tambah Alamat Baru</h1>
        ...
        <form action="{{ route('customers.addresses.store', $customer->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="label" value="{{ old('label') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Contoh: Rumah, Kantor, Toko" required>
                ...
                <div class="space-y-1.5 flex items-end pb-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_primary" value="1" {{ old('is_primary') || $customer->addresses()->count() === 0 ? 'checked' : '' }} class="w-5 h-5 text-primary rounded border-outline-variant focus:ring-primary">
                        <span class="text-label-md font-medium text-on-surface">Jadikan sebagai Alamat Utama</span>
                    </label>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="recipient_name" value="{{ old('recipient_name', $customer->name) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" required>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $customer->phone) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" required>
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <textarea name="address" rows="3" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Nama jalan, gedung, RT/RW, dsb." required>{{ old('address') }}</textarea>
                ...
                <div class="space-y-1.5 md:col-span-1">
                    ...
                    <input type="text" name="postal_code" value="{{ old('postal_code') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-lg bg-primary text-white hover:opacity-90 font-label-lg transition-colors shadow-sm">Simpan Alamat</button>
            </div>
        </form>
    </div>
@endsection
