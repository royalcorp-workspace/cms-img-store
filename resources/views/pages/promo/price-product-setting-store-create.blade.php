...
@section('title', 'Create Store Pricing')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Create Store Pricing</h1>
    ...
    <form method="POST" action="{{ route('price-product-setting-store.store') }}">
        ...
        <div class="rounded-xl shadow-sm border border-outline-variant/30 p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="target_type" id="targetType" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white" onchange="toggleTargetSelect()">
                        <option value="product" {{ old('target_type', 'product') == 'product' ? 'selected' : '' }}>Product</option>
                        <option value="variant" {{ old('target_type', 'product') == 'variant' ? 'selected' : '' }}>Variant</option>
                    </select>
                </div>
            </div>
            ...
            <div class="space-y-1.5 mt-4" id="productSelect">
                ...
                <div class="relative">
                    <input type="text" id="productSearchInput" placeholder="Type product name..." autocomplete="off" value="{{ $products->firstWhere('id', old('product_id'))?->name ?? '' }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white">
            ...
            <div class="space-y-1.5 mt-4 hidden" id="variantSelect">
                ...
                <select name="variant_id" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white">
                    ...
                    @foreach($products as $product)
                        @foreach($product->variants as $variant)
                            <option value="{{ $variant->id }}" {{ old('variant_id') == $variant->id ? 'selected' : '' }} data-product-id="{{ $product->id }}">{{ $product->name }} - {{ $variant->variant_name ?? $variant->sku }}</option>
                        @endforeach
                    @endforeach
                </select>
        ...
        <div class="sticky bottom-0 border-t border-outline-variant flex justify-between items-center px-8 py-4 z-30 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
            ...
            <div class="flex gap-4">
                ...
                <button type="submit" class="px-10 py-3 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">Save Store Pricing</button>
            </div>
        </div>
    </form>
@endsection
