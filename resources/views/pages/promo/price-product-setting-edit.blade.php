                            ...
                            <div class="variant-row py-1.5 px-2 rounded-md {{ $stock > 0 ? 'hover:bg-surface-container-low' : 'opacity-50' }} transition-colors">
                                <div class="flex items-start gap-2">
                                    <label class="flex items-center flex-shrink-0 pt-0.5">
                                        <input type="checkbox" class="variant-checkbox w-3.5 h-3.5 rounded border-outline-variant text-primary focus:ring-primary/30" value="{{ $variant->id }}" name="variant_ids[]" {{ $stock > 0 && in_array($variant->id, $selectedVariantIds ?? []) ? 'checked' : '' }} {{ $disabled }}>
                                    </label>
                                    <div class="flex-1 min-w-0">
