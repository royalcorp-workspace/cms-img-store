...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Create Product Tag</h1>
    ...
    <form id="tagForm" action="{{ route('tags.store') }}" method="POST" class="w-full space-y-6">
        @csrf
        ...
        <div class="w-full max-w-3xl bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="tagName" name="name" value="{{ old('name') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-sm" placeholder="e.g. Best Seller, New Arrival, Sale" required>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="tagSlug" name="slug" value="{{ old('slug') }}" readonly class="w-full px-3 py-2 border border-outline-variant rounded-lg bg-surface-container-low/60 cursor-not-allowed text-on-surface-variant focus:outline-none text-sm" placeholder="Otomatis terisi dari nama tag">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-sm" placeholder="0">
