...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Edit Category</h1>
        ...
        <div class="flex items-center gap-3">
            ...
            <button type="submit" form="categoryEditForm" class="btn-save inline-flex items-center gap-2 px-5 py-2 bg-primary text-white hover:opacity-90 rounded-xl text-xs font-bold transition-all shadow-sm active:scale-95">
    ...
    <form id="categoryEditForm" action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="w-full space-y-6" data-folder="categories">
        @csrf
        @method('PUT')
        ...
        <div class="w-full bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="categoryName" name="name" value="{{ old('name', $category->name) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g. Kasur Busa" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="tagline" value="{{ old('tagline', $category->tagline) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g. Solusi tidur berkualitas dan nyaman">
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" rows="3" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Tuliskan deskripsi lengkap tentang kategori ini...">{{ old('description', $category->description) }}</textarea>
            ...
            <div class="space-y-4">
                <div class="space-y-1.5">
                    ...
                    <select id="courier_setting_type" name="courier_setting_type" onchange="toggleCourierOptions()" class="w-full md:w-1/2 px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white text-sm">
                        <option value="detail" {{ old('courier_setting_type', $category->courier_setting_type ?? 'detail') === 'detail' ? 'selected' : '' }}>Detail / Kondisional (Kurir ditentukan per masing-masing produk)</option>
                        <option value="global" {{ old('courier_setting_type', $category->courier_setting_type) === 'global' ? 'selected' : '' }}>Global (Semua produk dalam kategori ini mengikuti aturan seragam di bawah)</option>
                    </select>
        ...
        <div class="w-full bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3 p-4 border border-outline-variant/30 rounded-xl bg-surface-container/20">
                    ...
                    <div id="previewBannerWeb" class="{{ $category->banner_web ? '' : 'hidden' }} mb-2">
                        <img src="{{ $category->banner_web_url }}" alt="Banner Web Preview" class="h-28 w-full object-cover rounded-lg border border-outline-variant shadow-sm">
                    </div>
                    ...
                    <p class="text-[11px] text-on-surface-variant">Biarkan kosong jika tidak ingin mengubah banner yang sudah ada.</p>
                ...
                <div class="space-y-3 p-4 border border-outline-variant/30 rounded-xl bg-surface-container/20">
                    ...
                    <div id="previewBannerMobile" class="{{ $category->banner_mobile ? '' : 'hidden' }} mb-2">
                        <img src="{{ $category->banner_mobile_url }}" alt="Banner Mobile Preview" class="h-28 w-full object-cover rounded-lg border border-outline-variant shadow-sm">
                    </div>
        ...
        <div class="w-full bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order ?? 0) }}" min="0" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none">
                    @error('sort_order') <span class="text-danger text-xs font-semibold">{{ $message }}</span> @enderror
                </div>

                <div class="flex items-center pt-6">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="status" value="1" {{ old('status', $category->is_active) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="flex items-center pt-6">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="has_warranty" value="1" {{ old('has_warranty', $category->has_warranty) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="flex items-center gap-3 pt-6 border-t border-outline-variant/20">
                <button type="submit" class="px-6 py-2.5 bg-primary text-white font-label-md hover:opacity-90 transition-all rounded-lg text-sm font-bold shadow-sm active:scale-95">Perbarui Kategori</button>
