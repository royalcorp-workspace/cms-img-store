...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="font-headline-lg text-headline-lg text-on-surface">Edit FAQ</h1>
...
<form method="POST" action="{{ route('content.faq.update', $faq->id) }}" class="space-y-6">
    @csrf
    @method('PUT')
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6">
        <div class="space-y-1.5">
            ...
            <input type="text" name="question" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Enter question..." required value="{{ old('question', $faq->question) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div class="space-y-1.5">
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $faq->sort_order) }}" min="0" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none">
            ...
            <div class="space-y-1.5">
                ...
                <select name="is_published" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white">
                    <option value="1" {{ old('is_published', $faq->is_published ?? '1') == '1' ? 'selected' : '' }}>Published</option>
                    <option value="0" {{ old('is_published', $faq->is_published ?? '1') == '0' ? 'selected' : '' }}>Draft</option>
                </select>
            </div>
        </div>
    </div>
