...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="font-headline-lg text-headline-lg text-on-surface">Blog Management</h1>
...
<div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
    <div class="p-4 border-b border-outline-variant flex flex-col sm:flex-row gap-3 justify-between">
        <form method="GET" class="flex items-center gap-2">
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search blog posts..." class="pl-9 pr-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary/20 focus:outline-none">
            </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-surface-gray">
                    ...
                    <th class="px-gutter py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Author</th>
                    <th class="px-gutter py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Status</th>
                    <th class="px-gutter py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Published</th>
            ...
            <tbody class="divide-y divide-outline-variant/20">
                ...
                <tr class="hover:bg-surface-container/30 transition-colors">
                    <td class="px-gutter py-4 font-body-md text-body-md text-on-surface font-semibold">{{ $post->title }}</td>
                    <td class="px-gutter py-4 font-body-md text-body-md text-on-surface-variant">{{ $post->author_name ?? '-' }}</td>
                    <td class="px-gutter py-4">
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-label-sm {{ $post->is_published ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning' }}">
                            <span class="w-1.5 h-1.5 rounded-full bg-current"></span> {{ $post->is_published ? 'Published' : 'Draft' }}
                        </span>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-gutter py-8 text-center text-on-surface-variant">No blog posts found</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="px-gutter py-4 border-t border-outline-variant bg-surface-container-low/30 flex justify-between items-center">
        <p class="font-body-md text-body-md text-on-surface-variant">Showing {{ $posts->firstItem() ?? 0 }}-{{ $posts->lastItem() ?? 0 }} of {{ number_format($posts->total()) }} entries</p>
        <div class="flex gap-2">{{ $posts->links() }}</div>
    </div>
</div>
@endsection
