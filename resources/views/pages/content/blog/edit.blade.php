...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="font-headline-lg text-headline-lg text-on-surface">Edit Blog Post</h1>
...
<form method="POST" action="{{ route('content.blog.update', $post->id) }}" class="space-y-6">
    @csrf
    @method('PUT')
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6">
        <div class="space-y-1.5">
            ...
            <input type="text" name="title" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Enter post title..." required value="{{ old('title', $post->title) }}">
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <input type="text" name="slug" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g., tips-tidur-sehat" required value="{{ old('slug', $post->slug) }}">
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <textarea name="excerpt" rows="2" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Short excerpt...">{{ old('excerpt', $post->excerpt) }}</textarea>
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <div id="quill-editor" style="min-height: 250px; font-size: 14px;">{!! old('content', $post->content) !!}</div>
            <textarea name="content" id="quill-content" class="opacity-0 absolute pointer-events-none h-0 w-0 -z-10">{{ old('content', $post->content) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div class="space-y-1.5">
                ...
                <div class="flex items-center gap-3">
                    <input type="text" name="featured_image" id="featuredImageUrl" class="flex-1 px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="https://..." value="{{ old('featured_image', $post->featured_image) }}">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="author_name" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Author name" value="{{ old('author_name', $post->author_name) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div class="space-y-1.5">
                ...
                <input type="date" name="published_at" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" value="{{ old('published_at', $post->published_at?->format('Y-m-d')) }}">
            ...
            <div class="space-y-1.5">
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $post->sort_order) }}" min="0" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none">
                @error('sort_order')<p class="text-danger text-sm">{{ $message }}</p>@enderror
            </div>
        </div>
        <div class="flex items-center gap-6 mt-4">
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }} class="w-4 h-4 rounded border-outline-variant text-primary focus:ring-primary">
                <span class="text-label-sm font-medium text-on-surface-variant">Published</span>
            </label>
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="w-4 h-4 rounded border-outline-variant text-primary focus:ring-primary">
    ...
    <div class="flex justify-end gap-4">
        ...
        <button type="submit" class="px-10 py-3 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">Update Post</button>
    </div>
</form>
