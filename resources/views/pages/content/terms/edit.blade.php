...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="font-headline-lg text-headline-lg text-on-surface">Edit Terms & Conditions</h1>
...
<form method="POST" action="{{ route('content.terms.update', $item->id) }}" class="space-y-6">
    @csrf
    @method('PUT')
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6">
        <div class="space-y-1.5">
            ...
            <input type="text" name="title" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Terms title..." required value="{{ old('title', $item->title) }}">
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <input type="text" name="slug" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g., syarat-dan-ketentuan" required value="{{ old('slug', $item->slug) }}">
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <textarea name="content" rows="10" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Full terms content..." required>{{ old('content', $item->content) }}</textarea>
    ...
    <div class="flex justify-end gap-4">
        ...
        <button type="submit" class="px-10 py-3 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">Update Terms</button>
    </div>
</form>
@endsection
