...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="font-headline-lg text-headline-lg text-on-surface">Warranty Claims</h1>
    ...
    <a href="{{ route('content.warranty.create') }}" class="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">
    ...
    <div class="p-4 border-b border-outline-variant flex flex-col sm:flex-row gap-3 justify-between">
        <form method="GET" class="flex items-center gap-2">
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search guides..." class="pl-9 pr-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary/20 focus:outline-none">
            </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-surface-gray">
                    ...
                    <th class="px-gutter py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Processing Time</th>
            ...
            <tbody class="divide-y divide-outline-variant/20">
                ...
                <tr class="hover:bg-surface-container/30 transition-colors">
                    ...
                    <td class="px-gutter py-4 font-body-md text-body-md text-on-surface-variant">{{ $item->processing_time_days ? $item->processing_time_days . ' days' : '-' }}</td>
                    ...
                    <td class="px-gutter py-4">
                        <div class="flex gap-2">
                            <a href="{{ route('content.warranty.edit', $item->id) }}" class="text-on-surface-variant hover:text-primary" title="Edit"><span class="material-symbols-outlined text-[18px]">edit</span></a>
                            <form action="{{ route('content.warranty.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this guide?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-gutter py-8 text-center text-on-surface-variant">No warranty guides found</td>
                </tr>
                @endforelse
            </tbody>
        </table>
