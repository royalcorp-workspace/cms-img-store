...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="font-headline-lg text-headline-lg text-on-surface">Create Warranty Claim</h1>
...
<form method="POST" action="{{ route('content.warranty.store') }}" id="warrantyForm" class="space-y-6">
    @csrf
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6">
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <input type="text" name="slug" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="e.g., cara-klaim-garansi" required value="{{ old('slug') }}">
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <textarea name="content" rows="6" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Full warranty guide content..." required>{{ old('content') }}</textarea>
        ...
        <div class="space-y-1.5 mt-4">
            ...
            <div id="documents-container" class="space-y-2">
                @php $initialDocs = old('required_documents') ? json_decode(old('required_documents'), true) : []; @endphp
                @if(!empty($initialDocs))
                    @foreach($initialDocs as $index => $doc)
                        <div class="document-item flex items-center gap-2" data-index="{{ $index }}">
                            <input type="text" name="document_name[]" value="{{ $doc['name'] ?? $doc ?? '' }}" class="document-name flex-1 px-3 py-2 border border-outline-variant rounded-lg text-body-sm focus:ring-2 focus:ring-primary/20 focus:outline-none" placeholder="Document name...">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div class="space-y-1.5">
                ...
                <input type="number" name="processing_time_days" value="{{ old('processing_time_days') }}" min="1" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none">
