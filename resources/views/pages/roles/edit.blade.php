...
<div class="mb-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="font-headline-xl text-headline-xl text-on-surface">Edit Hak Akses: {{ $role->name }}</h1>
            <p class="text-body-md text-on-surface-variant">Sesuaikan konfigurasi modul dan batasan aksi untuk role ini.</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <button type="submit" form="form-edit-role" class="flex items-center gap-2 px-5 py-2 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all shadow-sm">
...
<form id="form-edit-role" method="POST" action="{{ route('roles.update', $role->id) }}">
    @csrf
    @method('PUT')
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white p-6 rounded-xl shadow-subtle border border-surface-container">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required value="{{ old('name', $role->name) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg text-body-md focus:outline-none focus:border-primary">
                    </div>

                    <div>
                        <label for="level" class="block font-label-md text-label-md text-on-surface mb-1">Tingkatan (Level)</label>
                        <input type="number" name="level" id="level" min="1" max="100" value="{{ old('level', $role->level) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg text-body-md focus:outline-none focus:border-primary">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full px-3 py-2 border border-outline-variant rounded-lg text-body-md focus:outline-none focus:border-primary">{{ old('description', $role->description) }}</textarea>
                    </div>

                    @if($role->is_system)
                        <div class="p-3 bg-warning/10 border border-warning/30 rounded-lg flex items-center gap-2 text-warning text-body-sm">
                            <span class="material-symbols-outlined text-[18px]">lock</span>
                            <span>Ini adalah <strong>Role Sistem</strong> bawaan.</span>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-surface-container-low p-5 rounded-xl border border-surface-container">
                <h4 class="font-label-md text-label-md text-on-surface mb-2 font-bold">Ringkasan Hak Akses</h4>
                <div class="flex items-center justify-between text-body-sm text-on-surface-variant py-1">
                    <span>Total Hak Akses Terpasang:</span>
                    <strong class="text-primary font-bold">{{ count($rolePermissions) }}</strong>
                </div>
            </div>
        </div>
