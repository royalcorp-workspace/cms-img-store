...
@section('title', 'Handover Detail')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Handover #{{ substr($handover->id, 0, 8) }}</h1>
        ...
        <a href="{{ route('handover.index') }}" class="flex items-center gap-2 px-4 py-2 bg-surface-container text-on-surface rounded-lg font-label-md text-label-md hover:opacity-90 transition-all">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <p class="font-headline-md text-headline-md text-on-surface">#{{ substr($handover->order->id, 0, 8) }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Customer</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->order->customer->name ?? 'N/A' }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Warehouse</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->warehouse->name ?? '-' }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Courier</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->courier->name ?? '-' }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Tracking Number</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->tracking_number ?? '-' }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Driver</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->driver_name ?? '-' }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Driver Phone</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->driver_phone ?? '-' }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Status</p>
                <p class="font-body-md text-body-md text-on-surface">{{ ucfirst(str_replace('_', ' ', $handover->status)) }}</p>
            </div>
            <div>
                <p class="text-label-sm text-on-surface-variant mb-1">Handover At</p>
                <p class="font-body-md text-body-md text-on-surface">{{ $handover->handover_at ?? '-' }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-gray">
                        ...
                        <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Qty Handed Over</th>
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    @foreach($handover->items as $item)
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            ...
                            <td class="px-6 py-4 font-body-md text-body-md text-on-surface-variant">{{ $item->quantity_handed_over ?? '-' }}</td>
