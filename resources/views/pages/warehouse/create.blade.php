...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Tambah Gudang Baru</h1>
            <nav class="flex items-center gap-2 text-label-sm text-on-surface-variant mt-1 font-medium">
                ...
                <span class="material-symbols-outlined text-[14px]">chevron_right</span>
                <span class="text-on-surface">Tambah Gudang</span>
            </nav>
        </div>
        <div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden max-w-2xl">
        <div class="p-6 border-b border-outline-variant/30">
            <h2 class="text-base font-semibold text-on-surface">Informasi Gudang</h2>
            <p class="text-xs text-on-surface-variant mt-0.5">Lengkapi informasi gudang untuk manajemen alokasi stok.</p>
        </div>

        <form action="{{ route('warehouses.store') }}" method="POST" class="p-6 space-y-5">
            @csrf

            <div>
                <label class="block text-xs font-semibold text-on-surface mb-1.5">Kode Gudang <span class="text-danger">*</span></label>
                <input type="text" name="code" value="{{ old('code') }}" required placeholder="Contoh: GD-BDG01" class="w-full h-11 px-3 border border-outline-variant rounded-lg text-sm bg-white font-mono focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none uppercase">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Gudang Cabang Bandung" class="w-full h-11 px-3 border border-outline-variant rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none">
            </div>

            <div>
                <label class="block text-xs font-semibold text-on-surface mb-1.5">Kota</label>
                <input type="text" name="city" value="{{ old('city') }}" placeholder="Contoh: Kota Bandung" class="w-full h-11 px-3 border border-outline-variant rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none">
            </div>

            <div>
                <label class="block text-xs font-semibold text-on-surface mb-1.5">Alamat Lengkap</label>
                <textarea name="address" rows="3" placeholder="Alamat jalan, nomor, RT/RW, dsb." class="w-full p-3 border border-outline-variant rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none">{{ old('address') }}</textarea>
            </div>

            <div class="flex items-center gap-3 pt-2">
                <input type="checkbox" name="status" id="whStatus" value="1" {{ old('status', '1') == '1' ? 'checked' : '' }} class="w-4 h-4 text-primary border-outline-variant rounded focus:ring-primary/20">
