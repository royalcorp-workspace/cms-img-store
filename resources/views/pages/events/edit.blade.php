...
@section('title', 'Edit Campaign Event')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Edit Campaign Event</h1>
            <nav class="flex items-center gap-2 text-label-sm text-on-surface-variant mt-1 font-medium">
                ...
                <span class="material-symbols-outlined text-[14px]">chevron_right</span>
                <span class="text-on-surface">Edit: {{ $event->title }}</span>
            </nav>
        </div>
    ...
    <form id="eventForm" action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="w-full space-y-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="title" value="{{ old('title', $event->title) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md" required>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="event_type" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md" required>
                            <option value="mega_campaign" {{ old('event_type', $event->event_type ?? '') == 'mega_campaign' ? 'selected' : '' }}>Mega Campaign / Spesial</option>
                            <option value="flash_sale" {{ old('event_type', $event->event_type ?? '') == 'flash_sale' ? 'selected' : '' }}>Flash Sale (Terbatas Jam)</option>
                            <option value="clearance" {{ old('event_type', $event->event_type ?? '') == 'clearance' ? 'selected' : '' }}>Cuci Gudang (Clearance)</option>
                            <option value="new_arrival" {{ old('event_type', $event->event_type ?? '') == 'new_arrival' ? 'selected' : '' }}>Rilis Baru (New Arrival)</option>
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="slug" id="slug-input" value="{{ old('slug', $event->slug) }}" readonly class="w-full px-3 py-2 border border-outline-variant rounded-lg bg-surface-container/40 cursor-not-allowed focus:outline-none text-body-md text-on-surface-variant" placeholder="Will be auto-generated">
                    ...
                    <div class="space-y-1.5">
                        ...
                        @if(isset($event->banner_image) && $event->banner_image)
                            <div class="mb-2 w-32 h-20 bg-surface-container rounded-xl overflow-hidden border border-outline-variant/20 flex items-center justify-center bg-white p-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date', $event->start_date->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md" required>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="date" name="end_date" value="{{ old('end_date', $event->end_date->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md" required>
                        @error('end_date') <span class="text-danger text-xs font-semibold">{{ $message }}</span> @enderror
                    </div>

                    <div class="flex items-center pt-6">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ $event->is_active ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="pt-6 border-t border-outline-variant/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="popup_title" value="{{ old('popup_title', $event->popup->title ?? '') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md">
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-label-sm font-medium text-on-surface-variant">Action Redirect Link URL (CTA)</label>
                        <input type="text" name="popup_link" value="{{ old('popup_link', $event->popup->link_url ?? '') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        @if(isset($event->popup) && $event->popup->image_url)
                            <div class="mb-2 w-32 h-32 bg-surface-container rounded-xl border border-outline-variant/20 flex items-center justify-center bg-white p-1 overflow-hidden">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="popup_button_text" value="{{ old('popup_button_text', $event->popup->button_text ?? 'Lihat Promo') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none text-body-md">
                    </div>
                </div>

                <div class="pt-4 flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="popup_active" value="1" {{ isset($event->popup) && $event->popup->is_active ? 'checked' : '' }} class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-success"></div>
