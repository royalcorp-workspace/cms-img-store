        ...
        <div class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6">
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <select name="category_ids[]" multiple class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white select2-enable" data-placeholder="Pilih Kategori...">
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ in_array($cat->id, old('category_ids', $courier->restrictedCategories->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mt-4 border-t border-outline-variant/30 pt-4" x-data="{ search: '' }">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[400px] overflow-y-auto p-3 border border-outline-variant/50 rounded-xl bg-surface-gray/30 custom-scrollbar">
                    @php $selectedProductIds = old('product_ids', $courier->restrictedProducts->pluck('id')->toArray()); @endphp
                    @foreach($products as $prod)
                        <label class="bg-white rounded-lg border {{ in_array($prod->id, $selectedProductIds) ? 'border-primary/60 shadow-md bg-primary/5' : 'border-outline-variant/50 shadow-sm' }} flex items-center p-2.5 cursor-pointer hover:border-primary/50 hover:shadow-md transition-all gap-3" x-show="search === '' || '{{ strtolower(addslashes($prod->code . ' ' . $prod->name)) }}'.includes(search.toLowerCase())">
                            <input type="checkbox" name="product_ids[]" value="{{ $prod->id }}" {{ in_array($prod->id, $selectedProductIds) ? 'checked' : '' }} class="w-4 h-4 text-primary border-outline-variant rounded focus:ring-primary">
